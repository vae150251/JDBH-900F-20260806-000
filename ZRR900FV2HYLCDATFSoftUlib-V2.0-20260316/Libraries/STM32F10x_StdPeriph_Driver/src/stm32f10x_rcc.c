...
#else
#define CFGR_PLL_Mask            ((uint32_t)0x1FC0FFFF)
#endif /* STM32F10X_CL */

#define CFGR_PLLMull_Mask         ((uint32_t)0x603C0000)
...
#define CFGR_ADCPRE_Pos           ((uint32_t)14)
#define CFGR_ADCPREBit2_IdxMask   ((uint32_t)4)
#define CFGR_ADCPRE_Rst_Mask      ((uint32_t)~RCC_CFGR_ADCPRE)
#define CFGR_ADCPRE_Set_Mask      ((uint32_t)0x0000C000)
#define CFGR_ADCPREBit2_Set_Mask  ((uint32_t)0x10000000)
...
#ifndef STM32F10X_CL
  RCC->CFGR &= (uint32_t)0xE8FF0000;
#else
  RCC->CFGR &= (uint32_t)0xE0FF0000;
#endif /* STM32F10X_CL */
...
/**
  ...
  *   For @b other_STM32_devices, this parameter can be RCC_PLLMul_x where x:[2,64]
  ...
  * @retval None
  */
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t RCC_PLLMul, uint32_t RCC_PLLRange)
{
  uint32_t tmpreg = 0;
  ...
  tmpreg = RCC->CFGR;
  /* Clear PLLRANGE, PLLSRC, PLLXTPRE and PLLMUL[5:0] bits */
  tmpreg &= CFGR_PLL_Mask;
  /* Set the PLL configuration bits */
  tmpreg |= RCC_PLLSource | RCC_PLLMul | RCC_PLLRange;
  /* Store the new value */
  RCC->CFGR = tmpreg;
}
...
/**
  ...
  *     @arg RCC_USBCLKSource_PLLCLK_2Div5: PLL clock divided by 2,5 selected as USB
  *                                     clock source
  *     @arg RCC_USBCLKSource_PLLCLK_Div2: PLL clock divided by 2 selected as USB
																				clock source
  *     @arg RCC_USBCLKSource_PLLCLK_3Div5: PLL clock divided by 3,5 selected as USB
  *                                     clock source
...
void RCC_ADCCLKConfig(uint32_t RCC_PCLK2)
{
  ...
  tmpreg = RCC->CFGR;
  /* Clear ADCPRE[2:0] bits */
  tmpreg &= CFGR_ADCPRE_Rst_Mask;
  /* Set ADCPRE[2:0] bits according to RCC_PCLK2 value */
  tmpreg |= RCC_PCLK2;
  /* Store the new value */
  RCC->CFGR = tmpreg;
}
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks)
{
  ...
  /* Get HCLK prescaler */
  tmp = (RCC->CFGR & (RCC_CFGR_HPRE ^ RCC_CFGR_HPRE_3)) >> 4;
  presc = (RCC->CFGR & RCC_CFGR_HPRE_3) ? APBAHBPscTable[tmp] : 0;

  /* HCLK clock frequency */
  RCC_Clocks->HCLK_Frequency = RCC_Clocks->SYSCLK_Frequency >> presc;
  /* Get PCLK1 prescaler */
  tmp = (RCC->CFGR & (RCC_CFGR_PPRE1 ^ RCC_CFGR_PPRE1_2)) >> 8;
  presc = (RCC->CFGR & RCC_CFGR_PPRE1_2) ? APBAHBPscTable[tmp] : 0;
  /* PCLK1 clock frequency */
  RCC_Clocks->PCLK1_Frequency = RCC_Clocks->HCLK_Frequency >> presc;
  /* Get PCLK2 prescaler */
  tmp = (RCC->CFGR & (RCC_CFGR_PPRE2 ^ RCC_CFGR_PPRE2_2)) >> 11;
  presc = (RCC->CFGR & RCC_CFGR_PPRE2_2) ? APBAHBPscTable[tmp] : 0;
  /* PCLK2 clock frequency */
  RCC_Clocks->PCLK2_Frequency = RCC_Clocks->HCLK_Frequency >> presc;

  /* Get ADCCLK prescaler */
  tmp = (RCC->CFGR & CFGR_ADCPRE_Set_Mask) >> CFGR_ADCPRE_Pos;

  if (RCC->CFGR & CFGR_ADCPREBit2_Set_Mask)
  {
    tmp |= CFGR_ADCPREBit2_IdxMask;
  }

  presc = ADCPscTable[tmp];
...
/**
  ...
  *          RCC_APB1Periph_SPI4, RCC_APB1Periph_I2C3, RCC_APB1Periph_COMP
  * @param  NewState: new state of the specified peripheral clock.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
...
/**
  ...
  *          RCC_APB1Periph_PWR, RCC_APB1Periph_DAC, RCC_APB1Periph_CEC,
  *          RCC_APB1Periph_TIM12, RCC_APB1Periph_TIM13, RCC_APB1Periph_TIM14,
  *          RCC_APB1Periph_SPI4, RCC_APB1Periph_I2C3
  * @param  NewState: new state of the specified peripheral clock.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
...
/**
  ...
  *     @arg RCC_MCO_USB: USB clock selected
  *     @arg RCC_MCO_ADC: ADC clock selected
  *
  * @retval None
  */
...
void RCC_MCOConfig(uint32_t RCC_MCO)
{
  ...
  tmpreg &= ~RCC_CFGR_MCO;
  /* Set CLKOUT[2:0] bits according to RCC_MCO value */
  tmpreg |= (RCC_MCO & 0x07000000);
  /* Store the new value */
  RCC->CFGR = tmpreg;
}
#else
void RCC_MCOConfig(uint32_t RCC_MCO, uint32_t RCC_CLKOUTPRE)
{
  uint32_t tmpreg = 0;

  /* Check the parameters */
  assert_param(IS_RCC_MCO(RCC_MCO));
  assert_param(IS_RCC_MCO_PRE(RCC_CLKOUTPRE));
